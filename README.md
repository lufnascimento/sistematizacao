...
O motor CF0 1.2 já resolve a família contínua geométrica com orientação axial, fase física, raio conservador e verificadores, mas permanece `HYDRAULIC_UNCONFIRMED`. O estágio `C1_ALIGNMENT_SCREENING` gera sensibilidade TI limitada, e os núcleos `PCE0_RUSLE_SCREENING_ONLY` e `PCX0_EVENT_MASS_BALANCE_ONLY` agora tornam reproduzíveis a triagem RUSLE e o balanço volumétrico de um evento. Nenhum deles dimensiona seção, valida receptor ou autoriza guiamento. O próximo incremento é integrar evidência espacial do projeto, chuva-excesso, hidrogramas, propagação, seções e receptores para formar os motores PCE/PCX completos e, sobre eles, os solvers dimensionados C1, C2 e C3. Em paralelo, a campanha de aquisição deve fechar controle vertical, solo, chuva, bacia/receptores, interferências e frota; sem essa convergência, linhas suaves continuam sendo apenas candidatos topográficos. Projetos AgroCAD/Terraço/HidroTerraço e casos monitorados são benchmarks de paridade e validação, não fontes automáticas de verdade.
