...
O motor CF0 1.2 resolve a família contínua geométrica, mas permanece `HYDRAULIC_UNCONFIRMED`. O estágio `C1_ALIGNMENT_SCREENING` gera sensibilidade TI limitada; PCE0/PCX0 cobrem triagem RUSLE e balanço volumétrico; e a cadeia de chuva, hidrograma, propagação e capacidade compara seções novas, atuais e degradadas, limites declarados, borda livre e transbordamento. Ainda não há atenuação hidráulica, remanso, perdas em transições, simulação espacial do caminho excedente, validação do receptor ou autorização de guiamento. O próximo incremento é resolver essas condições de jusante e, sobre a cadeia verificada, formar os solvers dimensionados de curva embutida, base larga/passante e ESD.
