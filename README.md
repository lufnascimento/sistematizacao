...
O motor CF0 1.2 resolve a família contínua geométrica, mas permanece `HYDRAULIC_UNCONFIRMED`. O estágio `C1_ALIGNMENT_SCREENING` gera sensibilidade TI limitada; PCE0/PCX0 cobrem triagem RUSLE e balanço volumétrico; **Chuva que vira escoamento** gera os pulsos excedentes; **Hidrograma preliminar** calcula vazão no tempo; e **Propagação preliminar na rede** desloca e acumula essas vazões em um grafo declarado. Ainda não há atenuação, remanso, dimensionamento de seção, validação de receptor ou autorização de guiamento. O próximo incremento é associar geometria, perfil, seção e rugosidade aos trechos para testar capacidade nova e degradada e, sobre isso, formar os solvers dimensionados de curva embutida, base larga/passante e ESD.
