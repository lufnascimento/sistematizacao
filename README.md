...
O motor CF0 1.2 resolve a família contínua geométrica, mas permanece `HYDRAULIC_UNCONFIRMED`. O estágio `C1_ALIGNMENT_SCREENING` gera sensibilidade TI limitada; PCE0/PCX0 cobrem triagem RUSLE e balanço volumétrico; e `PCX1_NRCS_CN_RAINFALL_EXCESS_ONLY` agora gera chuva-excesso incremental de um evento e está acessível na plataforma por pedido imutável. Nenhum deles calcula vazão de pico, dimensiona seção, valida receptor ou autoriza guiamento. O próximo incremento é o hidrograma reproduzível com tempo de concentração/lag declarado, seguido de propagação, seções e receptores para formar o PCX completo e, sobre ele, os solvers dimensionados C1, C2 e C3. Em paralelo, a campanha de aquisição deve fechar controle vertical, solo, chuva, bacia/receptores, interferências e frota.
