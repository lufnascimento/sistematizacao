...
| PCE0, triagem de erosão | `IMPLEMENTADO_LIMITADO` | `pce_pcx.py`, runner estrito, schemas, exemplo analítico e testes calculam RUSLE com proveniência | não existe motor PCE espacial calibrado localmente; solo/cobertura/manejo não acompanham o pacote atual | fatores R, K, LS, C e P com fonte; tolerância local quando declarada | produto reproduzível; classificação somente com tolerância declarada; saída sempre `PCE0_RUSLE_SCREENING_ONLY` e nunca usada como PCX ou autorização |
| PCX0, balanço de massa | `IMPLEMENTADO_LIMITADO` | `pce_pcx.py`, runner estrito, schemas, exemplo analítico e testes fecham volumes por evento | recebe chuva-excesso pronta; não calcula método de excesso, hidrograma, propagação, seções, receptores ou falha | área contribuinte; intervalos; chuva-excesso; entradas, saídas e armazenamento | continuidade e balanço numérico reproduzíveis; saída sempre `PCX0_EVENT_MASS_BALANCE_ONLY`; `PASS` nunca implica capacidade hidráulica |
| PCE/PCX de projeto | `BLOQUEADO` | contratos, gates e os núcleos limitados PCE0/PCX0 estão disponíveis | faltam calibração espacial, chuva-excesso, hidrograma, propagação, seções, degradação, falha e receptores aprovados | solo/manejo espacial, IDF e distribuição temporal, Tc, infiltração, bacia completa, seções/rugosidade, sedimento, obstrução e jusante | suscetibilidade e concentração avaliadas; balanço de massa para eventos nominais/adversos; Q, nível, velocidade/tensão, bordo livre, receptor, sensibilidade e incerteza aprovados |
...
Hoje existem quatro recortes executáveis relevantes para esta matriz: a geometria CF0, sempre não hidráulica; o alinhamento C1 limitado; o estágio estático de POA, sempre isolado; e os núcleos PCE0/PCX0 de triagem. Os cenários dimensionados C1, C2 e C3 e os motores PCE/PCX completos são **especificações de produto**, não solvers executáveis. PCE0 não seleciona prática e PCX0 não calcula chuva-excesso, hidrograma, seção ou receptor.
